...
<tbody>
	...
	?>
		...
		<?php }else{ echo '<a href="'.$r['url'].'" target="_blank">查看</a>'; }?>
    ...
    <div class="btn"><label for="check_box"><?php echo L('selected_all');?>/<?php echo L('cancel');?></label>
		...
		<?php if(($category['istransful']&&$status==99)||empty($category['workflowid'])) { ?>
		<input type="hidden" name="modelid" value="<?php echo $category['modelid'];?>">
		<?php $jxsself = pc_base::load_config('jxsself'); if(!$jxsself['iscenter']){ ?><input type="hidden" name="data[jxsid]" value="<?php echo $jxsself['default']['jxs_id']; ?>"><?php  } ?>
		<input type="hidden" name="action" value="postcons">
		<input type="button" class="button" value="推送给官网" onclick="myform.action='?m=transinfos&c=index&a=dealsent';ckids();"/>
		<input type="button" class="button" value="同步到官网" onclick="myform.action='?m=transinfos&c=index&a=dealsent&data[update]=1';ckids();"/>
		<?php }?>
	</div>
