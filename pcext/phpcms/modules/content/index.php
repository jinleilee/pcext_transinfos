...
class index {
	...
	public function show() {
		...
		//add@20150104
		if($_GET['mid']){
			$modelid = intval($_GET['mid']);
			$MODEL = getcache('model','commons');
			$tablename = $this->db->table_name = $this->db->db_tablepre.$MODEL[$modelid]['tablename'];
			$r = $this->db->get_one(array('id'=>$id));
			$catid = $r['catid'];
		}
		!isset($catid) && $catid = intval($_GET['catid']);
		...
		!isset($MODEL) && $MODEL = getcache('model','commons');
		!isset($modelid) && $modelid = $CAT['modelid'];
		
		!isset($tablename) && $tablename = $this->db->table_name = $this->db->db_tablepre.$MODEL[$modelid]['tablename'];
		!isset($r) && $r = $this->db->get_one(array('id'=>$id));
		...
		$sec_parentid = $arrparentid[2] ? $arrparentid[2] : $catid;
		if($modelid==27){
			$sitelist  = getcache('sitelist','commons');
			$SEO['title'] = '【'.$title.'招聘】'.$sitelist[$siteid]['name'];
		}else{
			$SEO['title'] = $title.'_'.$CATEGORYS[$sec_parentid]['catname'].'_'.$CATEGORYS[2]['catname'];
		}
	...
	public function lists() {
		...
		//跳转
		if(in_array($catid,array(1,8,73,85,90,88,95,98))){
			$firstson4tz = explode(',',$arrchildid);
			$firstson4tz = $firstson4tz[1];
			header("Location:{$CATEGORYS[$firstson4tz]['url']}");
		}
		//跳转
		//SEO
		...
		if($type==0) {
			...
			!isset($categorydir) && $categorydir = $parentdir; //addby20150104
			$forefather = strpos($parentdir,'/');
			$GLOBALS['URL_ARRAY']['forefatherdir'] = substr($parentdir,0,$forefather+1);
