...
class url{
	...
	public function show($id, $page = 0, $catid = 0, $time = 0, $prefix = '',$data = '',$action = 'edit',$upgrade = 0) {
		...
		if($upgrade || (isset($_POST['upgrade']) && defined('IN_ADMIN') && $_POST['upgrade'])) {
			...
			$forefathernum = strpos($categorydir,'/');
			$forefatherdir = substr($categorydir,0,$forefathernum+1);
			$urls = str_replace(array('{$categorydir}','{$forefatherdir}','{$catdir}','{$year}','{$month}','{$day}','{$catid}','{$id}','{$page}'),array($categorydir,$forefatherdir,$catdir,$year,$month,$day,$catid,$id,$page),$urlrule);
