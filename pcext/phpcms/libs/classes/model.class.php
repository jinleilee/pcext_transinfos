...
class model {
	 ...
	 * 组合sql
	 */
	public function simp_cu($array,$type="add",$where = '') {
		$sql = ' '. $this->table_name . ' SET ' . $this->sqls($array,',');
		if($type == 'update'){
			if(is_array($where)) $where = ' where '.$this->sqls($where);
			return $this->db->query('UPDATE '.$sql.$where);
		}else{
			if($type == 'add'){
				$do = 'INSERT INTO ';
			}elseif($type == 'replace'){
				...
				return $this->mult_insert($array);
			}elseif($type == 'lots4id'){
				return $this->mult_insert($array,true);
			}elseif($type == 'lots4idreplace'){
				return $this->mult_insert($array,true,true);
			}else{
				return false;
			}
			$this->db->query($do.$sql);
			return $this->db->insert_id();
		}
	}
	/**
	 *一次插入多条数据
	 *@param $lists array(array('field0'=>'v0','field1'=>'v1'),++)
	 */
	 private function mult_insert($lists,$return_insert_id = false,$replace = false){
		if(empty($lists)){ return false;}
		
		foreach($lists as $array){
			if(!isset($insert['fields'])){
				$insert['fields'] = '(`'.implode('`,`',array_keys($array)).'`)';
			}
			$insert['values'][] = "('".implode("','",$array)."')";
		}
		$insert['values'] = implode(',',$insert['values']);
		$deal = $replace ? 'REPLACE INTO ' : 'INSERT INTO ' ;
		$sql = $deal.'`'.$this->table_name.'` '.$insert['fields'].' VALUES '.$insert['values'];
		$return = $this->db->query($sql);
		return $return_insert_id ? $this->insert_id() : $return;
	 }
	/**
	 *多条更新
	 *parame $values : array('name'=>array('name1','name2')++)
	 *parame $cases : array(array('id'=>'idv1','rank'=>'rankv1')++)
	 */
	 public function update_lots($values,$cases,$table=' '){
		...
		foreach($cases as $case_key => $case_fields){
			foreach($case_fields as $case_field => $case_value){
				!isset($ids_field) && $ids_field = $case_field;//for where ids
		...
		!empty($ids)&&$where = " WHERE `$ids_field` IN (".implode(',',$ids).") ";
